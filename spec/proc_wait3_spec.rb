...
RSpec.describe Process do
  ...
  example 'getrusage returns the expected struct' do
    ...
    @pid = fork{ sleep 0.5 }
    expect(described_class.getrusage).to be_kind_of(Struct::RUsage)
    expect(described_class.getrusage.stime).to be_kind_of(Float)
    expect(described_class.getrusage.utime).to be_kind_of(Float)
  end
